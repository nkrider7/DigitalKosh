        ...
        <motion.div variants={marqueeVariants}
          animate="animate" >
                <span className='font-gil text-yellow-300   flex justify-center gap-x-16'>{data && data.map((item, key) => (
                        ...
                        <div key={key} className='text-white  text-sm  w-full flex items-center gap-x-2'>#{item.item.market_cap_rank}<img src={item.item.small} className=' h-6 w-full rounded-full' alt="" />{item.item.name}
